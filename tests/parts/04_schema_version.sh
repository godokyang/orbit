...
"$CLI" new-task --task-type implementation --output "$SCHEMA_TASK" >/dev/null
...
# Test 3: Audit of evidence lacking schema_semantics blocks as removed legacy schema
# Build a minimal passing evidence + state setup for the audit command
LEGACY_SCHEMA_TASK="$TMPROOT/legacy-schema-task.yaml"
"$CLI" new-task --implementation-authority reviewer --assigned-instance reviewer-main --task-type slice_review --output "$LEGACY_SCHEMA_TASK" >/dev/null
...
json_assert 'audit of evidence without schema_semantics blocks as removed legacy schema' \
  "$TMPROOT/audit-legacy-semantics.json" \
  'j["trusted_for_done"] == false && (svs = j["schema_version_summary"]; svs.is_a?(Hash) && svs["legacy_errors"].is_a?(Array) && svs["legacy_errors"].any? { |w| w["kind"] == "removed_legacy_schema" && w["source"] == "evidence_file.schema_semantics" })'
...
"$CLI" new-task --implementation-authority reviewer --assigned-instance reviewer-main --task-type slice_review --output "$CONFLICT_TASK" >/dev/null
...
"$CLI" new-task --implementation-authority reviewer --assigned-instance reviewer-main --task-type slice_review --output "$SEMANTICS_TASK" >/dev/null
...
# Test 7: evidence submit of report missing report_template_version fails closed
...
ruby --disable-gems -ryaml -e 'p=ARGV[0]; y=YAML.safe_load(File.read(p), aliases: true); y.delete("report_template_version"); y.delete("schema_semantics"); File.write(p, YAML.dump(y))' "$LEGACY_REPORT"
expect_failure 'evidence submit of report without report_template_version fails closed' \
  env ORBIT_INSTANCE=reviewer-main "$CLI" evidence submit --file "$LEGACY_TEMPLATE_EVIDENCE" --report "$LEGACY_REPORT" --json
...
expect_failure 'evidence submit blocks on unknown future report_template_version' \
  env ORBIT_INSTANCE=reviewer-main "$CLI" evidence submit --file "$FUTURE_TEMPLATE_EVIDENCE" --report "$FUTURE_TEMPLATE_REPORT" --json
...
expect_failure 'evidence submit blocks when report_template_version does not match kind' \
  env ORBIT_INSTANCE=tester-main "$CLI" evidence submit --file "$KINDMISMATCH_EVIDENCE" --report "$KINDMISMATCH_REPORT" --json

# Test 10: current template version without schema_semantics fails closed
...
ruby --disable-gems -ryaml -e 'p=ARGV[0]; y=YAML.safe_load(File.read(p), aliases: true); y.delete("schema_semantics"); File.write(p, YAML.dump(y))' "$NOGAP_REPORT"
expect_failure 'current template version without schema_semantics fails closed' \
  env ORBIT_INSTANCE=reviewer-main "$CLI" evidence submit --file "$NOGAP_EVIDENCE" --report "$NOGAP_REPORT" --json

# Test 11: handoff includes schema_version_summary
HANDOFF_TASK="$TMPROOT/handoff-schema-task.yaml"
"$CLI" new-task --implementation-authority reviewer --assigned-instance reviewer-main --task-type slice_review --output "$HANDOFF_TASK" >/dev/null
...
ORBIT_INSTANCE=reviewer-main "$CLI" handoff --task "$HANDOFF_TASK" --state "$HANDOFF_STATE" --evidence "$HANDOFF_EVIDENCE" --json \
