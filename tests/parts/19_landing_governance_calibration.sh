...
"$CLI" new-task --implementation-authority reviewer --assigned-instance reviewer-main --task-type implementation_review --output "$S16_TASK" >/dev/null
...
"$CLI" new-task --task-type implementation --output "$S16_IMPL_TASK" >/dev/null
...
# ---- Group 2: warn_legacy policy is removed and fails validation ----
...
expect_failure 'validate rejects removed warn_legacy compatibility policy' "$CLI" validate --task "$S16_WARN_LEGACY_TASK" --json
pass 'warn_legacy policy is a removed compatibility mode'

# ---- Group 3: protocol change self-review blocked under strict risk ----

S16_STRICT_PROTO_TASK="$TMPROOT/s16-strict-proto-task.yaml"
"$CLI" new-task --task-type security_migration --output "$S16_STRICT_PROTO_TASK" >/dev/null
ruby --disable-gems -ryaml -e 'p=ARGV[0]; y=YAML.safe_load(File.read(p), aliases: true); y["compatibility_policy"]={"mode"=>"enforce_current","applies_to"=>["task","evidence"],"breaking_change"=>false,"migration_path"=>""}; y["self_review_guard"]={"protocol_changed"=>true,"independent_check_required"=>true,"same_system_self_approval_allowed"=>false}; File.write(p, YAML.dump(y))' "$S16_STRICT_PROTO_TASK"
...
"$CLI" new-task --task-type release_implementation --output "$S16_TOP_LEVEL_PROTO_TASK" >/dev/null
...
ruby --disable-gems -ryaml -e 'p=ARGV[0]; y=YAML.safe_load(File.read(p), aliases: true); y["compatibility_policy"]={"mode"=>"enforce_current","applies_to"=>["task","evidence"],"breaking_change"=>false,"migration_path"=>""}; y["self_review_guard"]={"protocol_changed"=>true,"independent_check"=>"reviewer-report:s16-top-level-protocol","independent_check_required"=>true,"same_system_self_approval_allowed"=>false}; y["release_readiness"]["ci"]={"provider"=>"github","run_id"=>"3","status"=>"passed"}; y["release_readiness"]["package"]={"artifact_path"=>"pkg.tgz","artifact_sha256"=>"c"*64,"contents_checked"=>true}; y["release_readiness"]["remote_state"]={"branch"=>"main","ahead_behind"=>"up_to_date"}; y["release_readiness"]["generated_artifacts"]=[{"path"=>"dist/app.js","checked"=>true}]; y["release_readiness"]["dogfood_suite"]={"status"=>"passed","case_ids"=>["stale-gate-detection"]}; File.write(p, YAML.dump(y))' "$S16_TOP_LEVEL_PROTO_OK_TASK"
