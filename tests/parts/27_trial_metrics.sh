...
json_assert 'paired task observations classify zero failure defect and question counts as observed_zero' metrics-zero-events-report.json 'counts=%w[workflow_failures independent_defects post_gate_user_defects status_questions]; counts.all? { |name| j.dig("coverage", name) == "observed_zero" && j.dig("denominators", "count_metrics", name, "observation_unit") == "paired_task" && j.dig("denominators", "count_metrics", name, "observed_tasks") == 1 && j.dig("denominators", "count_metrics", name, "events") == 0 } && j["observation_status"] == "ready_for_trial_decision"'
