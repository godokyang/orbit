...
json_assert 'unbound events stay outside the numerator and fail closed for decisions' metrics-zero-events-report.json 'counts=%w[workflow_failures independent_defects post_gate_user_defects status_questions]; counts.all? { |name| j.dig("coverage", name) == "observed_zero" && j.dig("denominators", "count_metrics", name, "observation_unit") == "paired_task" && j.dig("denominators", "count_metrics", name, "observed_tasks") == 1 && j.dig("denominators", "count_metrics", name, "events") == 0 } && j.dig("count_event_scope", "included_events") == 0 && j.dig("count_event_scope", "out_of_cohort_events") == 1 && j.dig("count_event_scope", "unbound_events") == 1 && j.dig("count_event_scope", "scope_status") == "ambiguous_event_scope" && j["observation_status"] == "ambiguous_event_scope"'

"$CLI" metrics capture --file .orbit/metrics/out-of-cohort-only.jsonl --task .orbit/tasks/trial.yaml --evidence .orbit/evidence/trial.json --stage baseline --duration-seconds 120 --tokens 500 --json >/dev/null
"$CLI" metrics capture --file .orbit/metrics/out-of-cohort-only.jsonl --task .orbit/tasks/trial.yaml --evidence .orbit/evidence/trial.json --stage after --duration-seconds 90 --tokens 350 --json >/dev/null
"$CLI" metrics record --file .orbit/metrics/out-of-cohort-only.jsonl --metric automatic_session --outcome verified --json >/dev/null
"$CLI" metrics record --file .orbit/metrics/out-of-cohort-only.jsonl --task .orbit/tasks/foreign-trial.yaml --metric workflow_failure --failure-kind identity --json >/dev/null
"$CLI" metrics report --file .orbit/metrics/out-of-cohort-only.jsonl --window-days 30 --json >metrics-out-of-cohort-report.json
json_assert 'bound out-of-cohort events are safely excluded from a ready decision' metrics-out-of-cohort-report.json 'counts=%w[workflow_failures independent_defects post_gate_user_defects status_questions]; counts.all? { |name| j.dig("coverage", name) == "observed_zero" } && j.dig("count_event_scope", "included_events") == 0 && j.dig("count_event_scope", "out_of_cohort_events") == 1 && j.dig("count_event_scope", "unbound_events") == 0 && j.dig("count_event_scope", "scope_status") == "resolved" && j["observation_status"] == "ready_for_trial_decision"'
