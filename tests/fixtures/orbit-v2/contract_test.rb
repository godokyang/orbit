...
require_relative "../../../lib/orbit/v2/gate_fact_store"
require_relative "../../../lib/orbit/v2/gate_engine"
...
module OrbitV2ContractTest
  ...
  def control_store_execution_records(bundle, policy, control_records, task_records, project_root,
    ...
    worker = OrbitV2FixtureFactory.agent(worker_id, role)
    rule_path = rule_path
    ...
    attempt = OrbitV2FixtureFactory.attempt(attempt_id, unit["work_unit_id"],
      worker["agent_instance_id"], role, purpose, thesis,
  ...
  def control_store_terminal_records(bundle, policy, control_records, task_records, execution,
    ...
    worker = OrbitV2FixtureFactory.agent(successor_worker_id, role)
    ...
    successor = OrbitV2FixtureFactory.attempt(successor_attempt_id, unit["work_unit_id"],
      ...
      predecessor_work_unit_attempt_ref:
        (successor_predecessor == :chained ? stored["attempt_id"] : successor_predecessor),
  ...
  def task_store_successor_records(parent, suffix: "main", extra_lineage: nil)
    task = OrbitV2FixtureFactory.deep_copy(parent[0])
    task["task_revision_id"] = "trev_inc6d#{suffix}_r2"
    task["revision_number"] = parent[0]["revision_number"] + 1
    task["parent_task_revision_id"] = parent[0]["task_revision_id"]
    task["protected_change_authorization_ref"] = nil
    task["authority_grant_refs"] = []
    gate = OrbitV2FixtureFactory.deep_copy(parent[1].first)
    gate["gate_requirement_id"] = "ogreq_inc6d#{suffix}_r2"
    gate["task_revision_id"] = task["task_revision_id"]
    gate["parent_gate_requirement_ref"] = {
      "gate_requirement_id" => parent[1].first["gate_requirement_id"],
      "content_digest" => parent[1].first["content_digest"]
    }
    gate["content_digest"] = Orbit::V2::CanonicalJSON.content_digest(gate)
    gates = [gate]
    if extra_lineage
      extra = OrbitV2FixtureFactory.deep_copy(gate)
      extra["gate_requirement_id"] = "ogreq_inc6d#{suffix}_extra"
      extra["gate_lineage_id"] = extra_lineage
      extra["parent_gate_requirement_ref"] = nil
      extra["content_digest"] = Orbit::V2::CanonicalJSON.content_digest(extra)
      gates << extra
    end
    task["gate_requirement_refs"] = gates.map { |candidate| candidate["gate_requirement_id"] }
    units = OrbitV2FixtureFactory.deep_copy(parent[2])
    unit_ids = {}
    units.each_with_index do |unit, index|
      unit_ids[unit["work_unit_id"]] = "owu_inc6d#{suffix}unit#{index + 1}"
    end
    units.each do |unit|
      unit["work_unit_id"] = unit_ids.fetch(unit["work_unit_id"])
      unit["task_revision_id"] = task["task_revision_id"]
      unit["parent_work_unit_ref"] = unit_ids[unit["parent_work_unit_ref"]]
      unit["depends_on_work_unit_refs"] = Array(unit["depends_on_work_unit_refs"]).map do |id|
        unit_ids.fetch(id)
      end
      unit["authority_scope"]["authorization_record_refs"] = []
    end
    theses = OrbitV2FixtureFactory.deep_copy(parent[3])
    theses.each_with_index do |thesis, index|
      thesis["change_thesis_id"] = "othesis_inc6d#{suffix}unit#{index + 1}"
      thesis["task_revision_id"] = task["task_revision_id"]
      thesis["work_unit_id"] = units[index]["work_unit_id"]
      thesis["content_digest"] = Orbit::V2::CanonicalJSON.content_digest(thesis)
      units[index]["initial_change_thesis_ref"] = {
        "change_thesis_id" => thesis["change_thesis_id"],
        "revision" => 1,
        "content_digest" => thesis["content_digest"]
      }
      units[index]["content_digest"] = Orbit::V2::CanonicalJSON.content_digest(units[index])
    end
    task["content_digest"] = Orbit::V2::CanonicalJSON.content_digest(task)
    assertions = []
    authorizations = []
    units.each do |unit|
      Array(unit.dig("authority_scope", "allowed_actions")).each do |action|
        built = OrbitV2FixtureFactory.work_authorization(unit, task, action)
        assertions << built.fetch("assertion")
        authorizations << built.fetch("record")
      end
    end
    [task, gates, units, theses, assertions, authorizations]
  end

  def task_store_successor(store, records, verifier)
    store.successor(task: records[0], gate_requirements: records[1], work_units: records[2],
      change_theses: records[3], authority_assertions: records[4],
      authorization_records: records[5], authority_verifier: verifier)
  end

  def task_store_authorize_successor(parent, successor, policy)
    task, gates, units, theses = successor[0, 4]
    task_record_id = "oauthz_inc6dtaskrisk"
    protected_record_id = "oauthz_inc6dprotected"
    task["authority_grant_refs"] = [task_record_id]
    task["protected_change_authorization_ref"] = protected_record_id
    task["content_digest"] = Orbit::V2::CanonicalJSON.content_digest(task)
    units.each { |unit| unit["authority_scope"]["authorization_record_refs"] = [] }
    work = units.flat_map do |unit|
      Array(unit.dig("authority_scope", "allowed_actions")).map do |action|
        OrbitV2FixtureFactory.work_authorization(unit, task, action)
      end
    end
    task_scope = Orbit::V2::TaskAuthority.scope_digest(task)
    task_assertion = OrbitV2FixtureFactory.assertion("oassert_inc6dtaskrisk",
      ["task.risk_authority.delegate"], "project-owner", authority_scope_ref: task_scope)
    task_record = OrbitV2FixtureFactory.digested(
      "schema_version" => "orbit-authorization-record-v1", "protocol_epoch" => "orbit-v2",
      "project_id" => task["project_id"], "authorization_record_id" => task_record_id,
      "project_policy_revision_id" => policy["policy_revision_id"],
      "action" => "task.risk_authority.delegate", "subject_ref" => task_scope,
      "authorization_source_ref" => task_assertion["assertion_id"],
      "authorization_assertion_digest" => task_assertion["assertion_digest"])
    change_digest = Orbit::V2::ProtectedChange.diff_digest(parent_task: parent[0],
      candidate_task: task, parent_gates: parent[1], candidate_gates: gates)
    protected_assertion = OrbitV2FixtureFactory.assertion("oassert_inc6dprotected",
      ["task.protected_contract.change"], "project-owner", authority_scope_ref: change_digest)
    receipt = protected_assertion.fetch("verification_receipt")
    envelope = {
      "schema_version" => "orbit-protected-change-authorization-v1",
      "project_id" => task["project_id"], "task_id" => task["task_id"],
      "parent_task_revision_ref" => { "task_revision_id" => parent[0]["task_revision_id"],
        "content_digest" => parent[0]["content_digest"] },
      "candidate_task_revision_ref" => { "task_revision_id" => task["task_revision_id"],
        "content_digest" => task["content_digest"] },
      "protected_change_digest" => change_digest,
      "authority_source_revision_ref" => { "provider_id" => protected_assertion["provider_id"],
        "receipt_id" => receipt["receipt_id"], "assertion_id" => protected_assertion["assertion_id"],
        "assertion_digest" => protected_assertion["assertion_digest"] },
      "issuer_authority_ref" => { "assertion_id" => protected_assertion["assertion_id"],
        "assertion_digest" => protected_assertion["assertion_digest"] },
      "project_policy_revision_ref" => { "policy_revision_id" => policy["policy_revision_id"],
        "content_digest" => policy["content_digest"] }, "decision" => "approved",
      "issued_at" => receipt["issued_at"]
    }
    envelope["envelope_digest"] = Orbit::V2::ProtectedChange.envelope_digest(envelope)
    protected_record = OrbitV2FixtureFactory.digested(
      "schema_version" => "orbit-authorization-record-v1", "protocol_epoch" => "orbit-v2",
      "project_id" => task["project_id"], "authorization_record_id" => protected_record_id,
      "project_policy_revision_id" => policy["policy_revision_id"],
      "action" => "task.protected_contract.change", "subject_ref" => task["task_revision_id"],
      "authorization_source_ref" => protected_assertion["assertion_id"],
      "authorization_assertion_digest" => protected_assertion["assertion_digest"],
      "protected_change_envelope" => envelope)
    successor[4] = work.map { |pair| pair["assertion"] } + [task_assertion, protected_assertion]
    successor[5] = work.map { |pair| pair["record"] } + [task_record, protected_record]
    successor
  end
  ...
  def test_task_store_successors
    Dir.mktmpdir do |dir|
      bundle = OrbitV2FixtureFactory.valid_bundle
      store, _policy = task_store_seeded_root(dir, bundle)
      verifier = OrbitV2FixtureFactory.authority_verifier
      ...
      control_records = control_store_genesis_records(bundle, genesis[0],
        ...
        lead: genesis[6])
      assert(control_store_genesis(control, control_records, control_verifiers) == :appended,
        "control pins the accepted revision-1 task")
      successor = task_store_successor_records(genesis, extra_lineage: "ogline_inc6dshared")
      assert(task_store_successor(store, successor, verifier) == :appended,
        "unprotected exact successor commits")
      tip = store.resolve(task_id: genesis[0]["task_id"], authority_verifier: verifier)
      historical = store.resolve(task_id: genesis[0]["task_id"],
        task_revision_id: genesis[0]["task_revision_id"], authority_verifier: verifier)
      assert(tip.dig("task", "task_revision_id") == successor[0]["task_revision_id"] &&
        tip["task_revisions"].size == 2 &&
        historical.dig("task", "task_revision_id") == genesis[0]["task_revision_id"] &&
        historical.dig("logical_lead", "task_id") == genesis[0]["task_id"],
        "default resolve selects the unique tip while exact historical reads stay stable")
      assert(control_store_resolve(control, "olcontrol_inc6dhistorical", control_verifiers)
        .dig("registry", "owned_task_refs", 0, "task_revision_id") == genesis[0]["task_revision_id"],
        "a proposed successor does not reinterpret an existing control's exact task revision")
      assert(task_store_successor(store, successor, verifier) == :idempotent,
        "successor replay is idempotent after full verification")
      ...
      expect_contract_error("task_store_genesis_invalid") do
        task_store_successor(store, forked, verifier)
      end
      wrong_gate = OrbitV2FixtureFactory.deep_copy(successor)
      wrong_gate[1][0]["parent_gate_requirement_ref"]["content_digest"] = "sha256:#{'0' * 64}"
      wrong_gate[1][0]["content_digest"] = Orbit::V2::CanonicalJSON.content_digest(wrong_gate[1][0])
      expect_contract_error("task_store_reuse") do
        task_store_successor(store, wrong_gate, verifier)
      end
      thesis = OrbitV2FixtureFactory.deep_copy(successor[3].first)
      thesis["revision"] = 2
      thesis["observed_problem"] = "The accepted first thesis needs a narrower causal claim."
      thesis["content_digest"] = Orbit::V2::CanonicalJSON.content_digest(thesis)
      assert(store.revise_thesis(change_thesis: thesis, authority_verifier: verifier) == :appended &&
        store.revise_thesis(change_thesis: thesis, authority_verifier: verifier) == :idempotent,
        "ChangeThesis successor is contiguous and replay-safe")
      skipped = OrbitV2FixtureFactory.deep_copy(thesis)
      skipped["revision"] = 4
      skipped["content_digest"] = Orbit::V2::CanonicalJSON.content_digest(skipped)
      expect_contract_error("task_store_genesis_invalid") do
        store.revise_thesis(change_thesis: skipped, authority_verifier: verifier)
      end
      resolved = store.resolve(task_id: genesis[0]["task_id"], authority_verifier: verifier)
      assert(resolved["change_theses"].count do |candidate|
        candidate["change_thesis_id"] == thesis["change_thesis_id"]
      end == 2 && store.records.size == 3,
        "resolved revision preserves the thesis lineage without changing the WorkUnit initial ref")
      policy = bundle["project_policy_revisions"].first
      rotated, issuance = policy_store_successor_pair(
        policy, "opolicy_inc6drotated", "oassert_inc6drotation"
      )
      Orbit::V2::PolicyStore.new(active_root: File.join(dir, ".orbit")).rotate(
        policy: rotated, assertion: issuance, authority_verifier: verifier)
      stale_thesis = OrbitV2FixtureFactory.deep_copy(thesis)
      stale_thesis["revision"] = 3
      stale_thesis["content_digest"] = Orbit::V2::CanonicalJSON.content_digest(stale_thesis)
      expect_contract_error("task_store_genesis_invalid") do
        store.revise_thesis(change_thesis: stale_thesis, authority_verifier: verifier)
      end
    end
    Dir.mktmpdir do |dir|
      bundle = OrbitV2FixtureFactory.valid_bundle
      ...
      verifier = OrbitV2FixtureFactory.authority_verifier
      genesis = task_store_genesis_records(bundle)
      task_store_genesis(store, genesis, verifier)
      protected = task_store_authorize_successor(
        genesis, task_store_successor_records(genesis, suffix: "protected"), policy
      )
      assert(task_store_successor(store, protected, verifier) == :appended,
        "exact protected-change envelope and TaskAuthority scope authorize a successor")
      forged = OrbitV2FixtureFactory.deep_copy(protected)
      forged[5].last.dig("protected_change_envelope")["protected_change_digest"] = "sha256:#{'0' * 64}"
      forged[5].last.dig("protected_change_envelope")["envelope_digest"] =
        Orbit::V2::ProtectedChange.envelope_digest(forged[5].last["protected_change_envelope"])
      forged[5].last["content_digest"] = Orbit::V2::CanonicalJSON.content_digest(forged[5].last)
      expect_contract_error("task_store_reuse") { task_store_successor(store, forged, verifier) }
    end
  end
  ...
  def gate_fact_accept(store, evaluation, findings, verifiers)
    store.accept(evaluation: evaluation, findings: findings,
      authority_verifier: verifiers[0], runtime_identity_verifier: verifiers[1],
      lifecycle_verifier: verifiers[2])
  end

  def gate_fact_resolve(store, evaluation_id, verifiers)
    store.resolve(gate_evaluation_id: evaluation_id, authority_verifier: verifiers[0],
      runtime_identity_verifier: verifiers[1], lifecycle_verifier: verifiers[2])
  end

  def gate_fact_resolve_finding(store, finding_id, verifiers)
    store.resolve_finding(finding_id: finding_id, authority_verifier: verifiers[0],
      runtime_identity_verifier: verifiers[1], lifecycle_verifier: verifiers[2])
  end
  ...
  def test_control_store_gate_facts
    ...
    Dir.mktmpdir do |dir|
      ...
      control_records = control_store_genesis_records(bundle, task_records[0],
        OrbitV2FixtureFactory.agent("oagent_inc6ggatelead", "lead"),
        "olcontrol_inc6ggate", control_store_writer_assertion("olcontrol_inc6ggate"),
        lead: task_records[6])
      assert(control_store_genesis(control, control_records, vf) == :appended,
        "gate control commits")
      impl1 = control_store_execution_records(bundle, policy, control_records, task_records, dir,
        attempt_id: "oattempt_inc6gimpl", dispatch_id: "olcheckpoint_inc6gimpldispatch",
        worker_id: "oagent_inc6gimplworker", rule_path: "rules/inc6g-coder.md")
      assert(control_store_dispatch(control, impl1, vf) == :appended,
        "subject implementation Attempt commits")
      snapshot = bundle["repository_snapshot"]
      paths = ["contracts/orbit-v2", "lib/orbit/v2"]
      ...
      thesis_for = lambda do |unit|
        task_records[3].find { |candidate| candidate["work_unit_id"] == unit["work_unit_id"] }
      end
      thesis0 = thesis_for.call(unit0)
      impl1_record = OrbitV2FixtureFactory.implementation_evidence(
        "oevr_inc6gimpl", impl1[3], impl1[0],
        { "change_thesis_id" => thesis0["change_thesis_id"], "revision" => thesis0["revision"],
          "content_digest" => thesis0["content_digest"] }, ["lib/orbit/v2/validator.rb"])
      assert(accept_evidence.call(impl1_record) == :appended,
        "subject evidence accepts while the Attempt is active")
      review_terminal = control_store_terminal_records(bundle, policy, control_records,
        task_records, impl1, dir, checkpoint_id: "olcheckpoint_inc6gimplterminal",
        successor_attempt_id: "oattempt_inc6greview",
        successor_worker_id: "oagent_inc6greviewer",
        rule_path: "rules/inc6g-review.md",
        role: "reviewer", purpose: "review", unit: review_unit,
        successor_predecessor: nil,
        successor_started_at: "2026-07-30T02:40:00Z",
        terminal_ended_at: "2026-07-30T02:30:00Z")
      assert(control_store_terminal(control, review_terminal, vf) == :appended,
        "the implementation round terminalizes into the same-unit evaluator round")
      review = [review_terminal[5], review_terminal[1], review_terminal[2],
        review_terminal[3], review_terminal[4], review_terminal[6], review_terminal[7]]
      submission = OrbitV2FixtureFactory.evaluator_submission(
        "oevr_inc6greview", review[3], review[0])
      assert(accept_evidence.call(submission) == :appended, "evaluator submission accepts")
      accepted_impl1 = evidence_store.resolve(evidence_record_id: "oevr_inc6gimpl",
        authority_verifier: vf[0], runtime_identity_verifier: vf[1], lifecycle_verifier: vf[2])
        .fetch("evidence_record")
      gate = task_records[1].first
      subject = Orbit::V2::EvaluationSubject.select(
        gate_requirement: gate, task_revision: task_records[0],
        work_units: task_records[2], attempts: [review_terminal[0]],
        evidence_records: [accepted_impl1],
        repository_snapshot: snapshot, code_surface: surface)
      evidence_ids = subject["evidence_record_refs"].map { |ref| ref["evidence_record_id"] }
      finding_id = "ofinding_inc6gblocking"
      evaluation = OrbitV2FixtureFactory.digested(
        "schema_version" => "orbit-gate-evaluation-v1",
        "protocol_epoch" => "orbit-v2",
        "project_id" => OrbitV2FixtureFactory::PROJECT_ID,
        "gate_evaluation_id" => "ogeval_inc6greview",
        ...
        "quality_question_answers" => [
          { "question_id" => "question_contract_quality", "verdict" => "pass",
            "evidence_record_refs" => evidence_ids }
        ],
        "acceptance_results" => [
          { "acceptance_id" => "acc_contract_valid", "verdict" => "fail",
            "evidence_record_refs" => evidence_ids }
        ],
        "counterexample_cases" => ["A malformed gate fact bundle is rejected."],
      ...
      finding = OrbitV2FixtureFactory.digested(
        ...
        "body" => "Example blocking gate concern.",
      ...
      resolved = gate_fact_resolve(gate_store, "ogeval_inc6greview", vf)
      assert(resolved.dig("gate_evaluation", "gate_evaluation_id") == "ogeval_inc6greview" &&
        gate_fact_resolve_finding(gate_store, finding_id, vf)["finding_id"] == finding_id,
        "exact readers return the accepted evaluation and finding by id")
      assert(gate_fact_accept(gate_store, evaluation, [finding], vf) == :idempotent,
        "gate fact replay is idempotent after full reverification")
      pass_evaluation = OrbitV2FixtureFactory.deep_copy(evaluation)
      pass_evaluation["gate_evaluation_id"] = "ogeval_inc6gpass"
      pass_evaluation["verdict"] = "pass"
      pass_evaluation["quality_outcome_verdict"] = "pass"
      pass_evaluation["acceptance_results"] = [
        { "acceptance_id" => "acc_contract_valid", "verdict" => "pass",
          "evidence_record_refs" => evidence_ids }
      ]
      pass_evaluation["coverage"] = {
        "summary" => "All implementation WorkUnits and evidence records were evaluated.",
        "covered_work_unit_refs" => subject["work_unit_refs"].map { |ref| ref["work_unit_id"] },
        "uncovered_work_unit_refs" => [],
        "evidence_record_refs" => evidence_ids
      }
      pass_evaluation["finding_refs"] = []
      pass_evaluation["content_digest"] = Orbit::V2::CanonicalJSON.content_digest(pass_evaluation)
      assert(gate_fact_accept(gate_store, pass_evaluation, [], vf) == :appended,
        "a pass GateEvaluation with an empty create-only Finding set commits atomically")
      re_target = lambda do |evaluation_id|
        paired_evaluation = OrbitV2FixtureFactory.deep_copy(evaluation)
        paired_evaluation["gate_evaluation_id"] = evaluation_id
        paired_finding = OrbitV2FixtureFactory.deep_copy(finding)
        paired_finding["finding_id"] = "ofinding_#{evaluation_id.delete_prefix('ogeval_')}"
        paired_finding["gate_evaluation_id"] = evaluation_id
        paired_evaluation["finding_refs"] = [paired_finding["finding_id"]]
        paired_evaluation["content_digest"] =
          Orbit::V2::CanonicalJSON.content_digest(paired_evaluation)
      ...
      evaluator_terminal = control_store_terminal_records(bundle, policy, control_records,
        ...
        successor_worker_id: "oagent_inc6greviewsuccessorworker",
        rule_path: "rules/inc6g-review2.md", terminal_ended_at: "2026-07-30T03:10:00Z",
        role: "reviewer", purpose: "review", unit: review_unit,
      ...
      rotated, issuance = policy_store_successor_pair(
        policy, "opolicy_inc6grotated", "oassert_inc6grotated")
      assert(Orbit::V2::PolicyStore.new(active_root: File.join(dir, ".orbit")).rotate(
        policy: rotated, assertion: issuance, authority_verifier: vf[0]) == :appended,
        "policy rotates after the evaluator terminal")
      assert(gate_fact_resolve(gate_store, evaluation["gate_evaluation_id"], vf)
        .dig("gate_evaluation", "content_digest") == evaluation["content_digest"] &&
        gate_fact_resolve_finding(gate_store, finding_id, vf)["finding_id"] == finding_id,
        "historical gate facts remain exact-readable after policy rotation")
      changed = OrbitV2FixtureFactory.deep_copy(evaluation)
      changed["verdict"] = "blocked"
      changed["content_digest"] = Orbit::V2::CanonicalJSON.content_digest(changed)
      expect_contract_error("gate_facts_reuse") { gate_fact_accept(gate_store, changed, [finding], vf) }
      second_finding = OrbitV2FixtureFactory.deep_copy(finding)
      second_finding["finding_id"] = finding_id
      second_finding["content_digest"] = Orbit::V2::CanonicalJSON.content_digest(second_finding)
      second_evaluation = OrbitV2FixtureFactory.deep_copy(evaluation)
      second_evaluation["gate_evaluation_id"] = "ogeval_inc6gsecond"
      second_evaluation["finding_refs"] = [finding_id]
      second_evaluation["content_digest"] = Orbit::V2::CanonicalJSON.content_digest(second_evaluation)
      expect_contract_error("gate_facts_acceptance_invalid") do
        gate_fact_accept(gate_store, second_evaluation, [second_finding], vf)
      end
      wrong_refs = OrbitV2FixtureFactory.deep_copy(evaluation)
      wrong_refs["gate_evaluation_id"] = "ogeval_inc6gwrongrefs"
      wrong_refs["finding_refs"] = ["ofinding_inc6gborrowed"]
      wrong_refs["content_digest"] = Orbit::V2::CanonicalJSON.content_digest(wrong_refs)
      expect_contract_error("gate_facts_acceptance_invalid") do
        gate_fact_accept(gate_store, wrong_refs, [finding], vf)
      end
      phantom = OrbitV2FixtureFactory.deep_copy(finding)
      phantom["finding_id"] = "ofinding_inc6gphantom"
      phantom["source_evidence_record_refs"] = ["oevr_inc6gphantom"]
      phantom["content_digest"] = Orbit::V2::CanonicalJSON.content_digest(phantom)
      stale_evaluation = OrbitV2FixtureFactory.deep_copy(evaluation)
      stale_evaluation["gate_evaluation_id"] = "ogeval_inc6gphantom"
      stale_evaluation["finding_refs"] = [phantom["finding_id"]]
      stale_evaluation["content_digest"] = Orbit::V2::CanonicalJSON.content_digest(stale_evaluation)
      expect_contract_error("gate_facts_acceptance_invalid") do
        gate_fact_accept(gate_store, stale_evaluation, [phantom], vf)
      end
      stale_gate = OrbitV2FixtureFactory.deep_copy(evaluation)
      stale_gate["gate_evaluation_id"] = "ogeval_inc6gstale"
      stale_gate["gate_requirement_content_digest"] = "sha256:#{'0' * 64}"
      stale_gate["content_digest"] = Orbit::V2::CanonicalJSON.content_digest(stale_gate)
      expect_contract_error("gate_facts_acceptance_invalid") do
      ...
      expect_contract_error("gate_facts_lineage_invalid") do
        gate_fact_resolve(gate_store, "ogeval_inc6greview", vf)
      end
    end
  end
  ...
  def test_gate_engine
    assert(Orbit::V2::GateEngine.instance_methods(false).sort == [:derive],
      "GateEngine exposes exactly its single derive seam")
    Dir.mktmpdir do |dir|
      bundle = OrbitV2FixtureFactory.valid_bundle
      review_bundle_unit = bundle["work_units"].fetch(2)
      review_bundle_unit["depends_on_work_unit_refs"] = [bundle["work_units"].first["work_unit_id"]]
      review_bundle_unit["content_digest"] = Orbit::V2::CanonicalJSON.content_digest(review_bundle_unit)
      control, policy, = control_store_seeded_root(dir, bundle)
      vf = control_verifiers
      task_records = task_store_genesis_records(bundle)
      gate = task_records[1].first
      ...
      assert(control_store_genesis(control, control_records, vf) == :appended, "control commits")
      impl = control_store_execution_records(bundle, policy, control_records, task_records, dir,
        attempt_id: "oattempt_inc6iimpl", dispatch_id: "olcheckpoint_inc6iimpldispatch",
        worker_id: "oagent_inc6iimplworker", rule_path: "rules/inc6i-coder.md")
      assert(control_store_dispatch(control, impl, vf) == :appended, "implementation round commits")
      snapshot = bundle["repository_snapshot"]
      review_unit = task_records[2].fetch(2)
      review_terminal = control_store_terminal_records(bundle, policy, control_records,
        task_records, impl, dir, checkpoint_id: "olcheckpoint_inc6iimplterminal",
        successor_attempt_id: "oattempt_inc6ireview", successor_worker_id: "oagent_inc6ireviewer",
        rule_path: "rules/inc6i-review.md", role: "reviewer", purpose: "review", unit: review_unit,
        successor_predecessor: nil, successor_started_at: "2026-07-30T02:40:00Z",
        terminal_ended_at: "2026-07-30T02:30:00Z")
      assert(control_store_terminal(control, review_terminal, vf) == :appended,
        "the review round succeeds the implementation round")
      review = [5, 1, 2, 3, 4, 6, 7].map { |index| review_terminal[index] }
      unit0 = task_records[2].first
      thesis0 = task_records[3].find { |candidate| candidate["work_unit_id"] == unit0["work_unit_id"] }
      ...
      impl_record = OrbitV2FixtureFactory.implementation_evidence("oevr_inc6iimpl", impl[3], impl[0],
        { "change_thesis_id" => thesis0["change_thesis_id"], "revision" => thesis0["revision"], "content_digest" => thesis0["content_digest"] },
        ["lib/orbit/v2/validator.rb"])
      accept_evidence.call(impl_record)
      review_submission = OrbitV2FixtureFactory.evaluator_submission("oevr_inc6ireview", review[3], review[0])
      accept_evidence.call(review_submission)
      followup_submission = OrbitV2FixtureFactory.evaluator_submission("oevr_inc6ifollowup", review[3], review[0])
      accept_evidence.call(followup_submission)
      accepted_impl = evidence_store.resolve(evidence_record_id: "oevr_inc6iimpl",
        authority_verifier: vf[0], runtime_identity_verifier: vf[1], lifecycle_verifier: vf[2]).fetch("evidence_record")
      accepted_review = evidence_store.resolve(evidence_record_id: "oevr_inc6ireview",
        authority_verifier: vf[0], runtime_identity_verifier: vf[1], lifecycle_verifier: vf[2]).fetch("evidence_record")
      accepted_followup = evidence_store.resolve(evidence_record_id: "oevr_inc6ifollowup",
        authority_verifier: vf[0], runtime_identity_verifier: vf[1], lifecycle_verifier: vf[2]).fetch("evidence_record")
      surface = Orbit::V2::CodeSurface.derive(repository_snapshot: snapshot, paths: %w[contracts/orbit-v2 lib/orbit/v2])
      subject = Orbit::V2::EvaluationSubject.select(gate_requirement: gate,
        task_revision: task_records[0], work_units: task_records[2], attempts: [review_terminal[0]],
        evidence_records: [accepted_impl, accepted_review], repository_snapshot: snapshot, code_surface: surface)
      risk_finding_id = "ofinding_inc6irisk"
      risk = OrbitV2FixtureFactory.deep_copy(bundle["gate_evaluations"].first)
      risk.merge!("gate_evaluation_id" => "ogeval_inc6irisk",
        "supersedes_gate_evaluation_id" => nil,
        "evaluator_attempt_id" => review[3]["attempt_id"],
        "evaluator_submission_record_id" => review_submission["evidence_record_id"],
        "subject" => subject, "verdict" => "fail", "quality_outcome_verdict" => "fail",
        "counterexample_cases" => ["A stale gate decision must never decide."],
        "residual_risk" => "The old evaluation predates the follow-up evidence.",
        "finding_refs" => [risk_finding_id])
      risk["gate_requirement_content_digest"] = gate["content_digest"]
      risk_evidence_ids = subject["evidence_record_refs"].map { |ref| ref["evidence_record_id"] }
      risk["quality_question_answers"].each { |answer| answer["evidence_record_refs"] = risk_evidence_ids }
      risk["acceptance_results"].each { |result| result["evidence_record_refs"] = risk_evidence_ids }
      risk["coverage"].merge!("covered_work_unit_refs" =>
        subject["work_unit_refs"].map { |ref| ref["work_unit_id"] },
        "evidence_record_refs" => risk_evidence_ids)
      risk["content_digest"] = Orbit::V2::CanonicalJSON.content_digest(risk)
      risk_finding = OrbitV2FixtureFactory.digested(
        "schema_version" => "orbit-finding-v1", "protocol_epoch" => "orbit-v2",
        "project_id" => OrbitV2FixtureFactory::PROJECT_ID,
        "finding_id" => risk_finding_id, "gate_evaluation_id" => risk["gate_evaluation_id"],
        "severity" => "P0", "basis" => "newly_discovered_risk",
        "body" => "Example unadjudicated risk.",
      ...
      finding_ref = { "kind" => "finding", "id" => risk_finding_id, "digest" => risk_finding["content_digest"] }
      escalate_assertion = OrbitV2FixtureFactory.assertion("oassert_inc6iescalate",
        %w[control.checkpoint], "control-plane-writer", authority_scope_ref: "olcontrol_inc6hgate")
      escalate = inc6h_checkpoint("olcheckpoint_inc6iescalate", review_terminal[6], policy,
        escalate_assertion, control_records, task_records, unit0,
        decision: { "state" => "needs_user", "action" => "escalate", "reason" => "newly discovered risk requires risk-owner/user adjudication" },
        reconcile_trigger: { "event" => "finding_change", "reason" => "Risk Finding accepted." },
        next_trigger: { "event" => "authority_change", "reason" => "Awaiting adjudication." },
        supporting_refs: [finding_ref])
      assert(control.checkpoint(checkpoint: escalate, assertion: escalate_assertion,
        authority_verifier: vf[0], runtime_identity_verifier: vf[1], lifecycle_verifier: vf[2]) == :appended,
        "the finding_change checkpoint observes the risk")
      current_subject = Orbit::V2::EvaluationSubject.select(gate_requirement: gate,
        task_revision: task_records[0], work_units: task_records[2], attempts: [review_terminal[0]],
        evidence_records: [accepted_impl, accepted_review, accepted_followup],
        repository_snapshot: snapshot, code_surface: surface)
      followup = OrbitV2FixtureFactory.deep_copy(risk)
      followup.merge!("gate_evaluation_id" => "ogeval_inc6iresolved", "verdict" => "pass",
        "quality_outcome_verdict" => "pass", "finding_refs" => [],
        "supersedes_gate_evaluation_id" => risk["gate_evaluation_id"],
        "evaluator_attempt_id" => review[3]["attempt_id"],
        "evaluator_submission_record_id" => followup_submission["evidence_record_id"],
        "subject" => current_subject)
      followup["acceptance_results"].each { |result| result["verdict"] = "pass" }
      followup["counterexample_cases"] = []
      followup["content_digest"] = Orbit::V2::CanonicalJSON.content_digest(followup)
      assert(gate_fact_accept(gate_store, followup, [], vf) == :appended,
        "a distinct follow-up GateEvaluation commits")
      resolution = OrbitV2FixtureFactory.digested(
        "schema_version" => "orbit-finding-resolution-v1",
        "protocol_epoch" => "orbit-v2", "project_id" => OrbitV2FixtureFactory::PROJECT_ID,
        "finding_resolution_id" => "ofres_inc6iaddressed", "finding_id" => risk_finding_id,
        "resolution" => "addressed", "issuer_attempt_id" => review[3]["attempt_id"],
        "issuer_submission_record_id" => followup_submission["evidence_record_id"],
        "source_finding_ref" => { "finding_id" => risk_finding_id, "content_digest" => risk_finding["content_digest"] },
        "source_gate_evaluation_ref" => { "gate_evaluation_id" => risk["gate_evaluation_id"],
          "content_digest" => risk["content_digest"] },
        "resolving_gate_evaluation_ref" => { "gate_evaluation_id" => followup["gate_evaluation_id"],
          "content_digest" => followup["content_digest"] },
        "proposal_evidence_record_id" => impl_record["evidence_record_id"],
        "supporting_record_refs" => [impl_record["evidence_record_id"]],
        "supersedes_finding_resolution_id" => nil)
      assert(gate_fact_resolution(gate_store, resolution, vf) == :appended,
      ...
      assert(reopened["content_digest"] == outcome["content_digest"],
        "a fresh engine reopens the same deterministic outcome")
      result = outcome["gate_results"].find do |candidate|
        candidate["gate_requirement_ref"]["gate_requirement_id"] == gate["gate_requirement_id"]
      end
      assert(result["status"] == "passed" &&
        result["gate_evaluation_ref"]["gate_evaluation_id"] == followup["gate_evaluation_id"],
        "the stale failing evaluation never decides; the current follow-up does")
      assert(outcome["closed"] == true && outcome["unresolved_adjudication_required_finding_refs"].empty?,
        "the addressed resolution closes the gate")
      manifest_ids = outcome["source_manifest"].map { |entry| entry["id"] }
      assert(manifest_ids.include?("ogeval_inc6iresolved") && manifest_ids.include?("ogeval_inc6irisk") &&
        manifest_ids.include?("ofinding_inc6irisk") && manifest_ids.include?("ofres_inc6iaddressed") &&
        manifest_ids.include?("oevr_inc6ifollowup"),
      ...
      expect_contract_error("gate_engine_argument_invalid") do
