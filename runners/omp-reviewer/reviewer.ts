...
try {
	if (!request.snapshot || !request.profile || !request.out) throw new Error("snapshot, profile, and out are required");
	const snapshot = path.resolve(request.snapshot);
	...
	if (within(snapshot, out) || within(snapshot, profile)) throw new Error("profile and out must stay outside the snapshot");
	const version = sdkVersion();
	evidence.sdk_version = version;
	if (version !== PINNED_SDK) throw new Error(`SDK ${version} is not the pinned ${PINNED_SDK}`);
	if (!request.model && !request.probe_outside) throw new Error("pass --model for a check, or a confinement probe; neither is a pass by itself");

	evidence.fingerprint_before = fingerprint(snapshot);
	...
	if (request.model) {
		...
		if (!model) throw new Error(`model not in catalog: ${request.model}`);
		...
		if (!key) throw new Error(`no credential for ${request.token_provider ?? provider}`);
...
if (request.out && !within(path.resolve(request.snapshot ?? "."), path.resolve(request.out))) {
