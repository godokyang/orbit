...
Zeen 页面走查任务的真实样本（任务 `5d58ce62-2c5a-46c0-b2c0-cfec3ac05fdf`：六次独立检查、无 `manual_check` 与 `finalization_notice` 事件、`finalization_notices` 为空）暴露的缺口是：Root 在最终版本没有当前有效终检的情况下用「交付完成」发起工具 `stop`；工具对 `stop` 一律附带内部 complete 标记，运行时因完成前置条件不满足直接落入普通停止、记 `paused`，调用方只收到「命令已入队」的 `queued` 回执。停止确认本身完整，但完成意图与实际结果不一致，开放 finding 留在已暂停任务中，用户看到的交付措辞与记录不符。诊断见 [Zeen 页面走查任务中的 Orbit 流程](../reference/zeen-page-capture-orbit-process-review-20260925.md)；实现与隔离真实验收的通过项、保留边界见 [Orbit OMP 接入验收记录](../reference/orbit-omp-access-acceptance-20260925.md)。

本条是与宿主无关的收尾与接入语义，已写入[任务运行合同](../../contracts/task-runtime.md)：

1. **完成意图不做静默降级。** Root 工具的完成意图在执行前同步核对当前版本的 manual reviewer 结论无遗留问题：需存在按当前产物与输入版本绑定的 `finalization_notice`、无 open finding 与待核对线索、成员已结束、检查可用。缺 `finalization_notice` 或仍有未闭合问题时，同步返回结构化拒绝与下一动作，不入队、不改记暂停、不改变任务状态；任务保持运行，用户之后仍可再次请求终检。
2. **暂停是独立意图。** Root 工具的暂停意图与普通 CLI `stop` 保持普通停止语义（`paused`／`stop_unconfirmed`，保留会话与上下文），不受完成前置条件约束；CLI 原有普通 stop 行为不变。
3. **二次核对保留。** 即使工具侧预检通过，程序在实际停止前仍核对当前产物、输入、成员与停止确认；任一不满足不得记 `complete`，等待超时或交付轮次被中断同样按普通停止处理。
4. **每轮状态摘要。** 已启动任务在该 Root 会话的每个用户回合开始前注入一条由任务持久记录派生的短摘要（当前阶段、开放问题、是否已有当前版本的有效终检、下一动作）；未启动任务只提示接入条件。摘要不是第二份状态、不冒充用户输入，任务事实仍以任务目录记录为准。
5. **接入边界不变。** `orbit omp` 只提供接入资格，不自动创建任务、不自动派发执行成员；启动与派发仍由 Root 显式决定，未启动任务不产生检查成本。

理由：完成与暂停是用户可见的不同结果，把「停止已排队」当成交付事实，会让未终检的版本看起来已被独立复核。在工具执行前同步拒绝并给出可执行的下一步，比事后靠 `status` 展示或 `session_stop` 续跑补救更可靠，也不把风险推给用户主动查状态；保留独立暂停意图，是为了不把用户明确中断变成受完成门约束的路径。自动建任务或自动派发成员对本缺口没有帮助，反而扩大产品范围，故不采纳。

边界与未完成项：OMP 18.2.8 的 `session_stop` 有界续跑不进入首轮实现，也不作为完成裁定权威。每轮摘要的具体字段与 `before_agent_start` 注入方式、回执区分「命令已入队」与「完成前置条件未满足」的实现已随源码落地（工作区提交前）；隔离 OMP 真实任务的验收（成员结果→自动 finding→Root 修正→手动终检→notice→显式 stop）仍在进行，通过项与保留边界见 [Orbit OMP 接入验收记录](../reference/orbit-omp-access-acceptance-20260925.md)，未验收前不得把相应行为当作已验收。

### 实现后的两点补充（2026-09-25，已进入现行合同）

实现硬门时发现两处决定需要写清，均已落地并写入[任务运行合同](../../contracts/task-runtime.md)：

1. **runtime 已退出的完成请求单独拒绝。** 记录的运行进程已退出或已失败时，完成意图没有任何观察者，工具同步返回 `runtime_unavailable`，并指引用一次**显式普通 stop** 走既有的已确认清理与成员收尾；该清理重试仍属普通停止路径，完成意图不得借用它，也不得降级为暂停而不告知。拒绝回执与运行时唤醒共用同一「原因码→下一动作」映射：缺 notice 指向在最终版本上请求手动 `check`，open finding／待核对线索先修正或完成核对，成员未结算先结束成员，检查清理未核实先重试并核实清理——不把无法直接重检的原因都说成再检查一次。
2. **拆资源后仍需一次当前版本／成员复核。** 成员停止要等在途工作，拆资源因此可能持续；此前的实现只在拆资源前算一次 notice 并沿用到最后，若停止期间产物改变就会误记 `complete`（旧实现在拆资源后复核，失效时只是静默暂停，同样不满足要求）。现行规则：拆资源后用**纯持久判据**再复核一次（notice 与版本的对应、open finding、待核对线索、成员结算、检查清理是否已核实；指纹或 roster 不可读同样不合格），**不再查询已停止 Root 的桥接**——停止证据由本次 `stop_confirmation` 判定，桥接瞬时不可达不得把合格完成降级成暂停。复核不合格时已停止的运行记 `paused`（停止确认失败的记 `stop_unconfirmed`），保留 `completion_invalidation`（原因码、说明、时间）与 `completion_invalidated_after_stop` 事件，绝不记 `complete`，也不得表述为任务仍在运行。复核还要重新读取权威 `members.json` 并与本次停止覆盖的名单对照：停止期间新登记进名单的成员从未被 `confirmed_stop`（它只停入口时同步的名单）停过，名单不可读同样无法证明停止完整，两种情形都不得记 `complete`，因成员未被实际停止而记 `stop_unconfirmed`，不得谎称 `paused` 已确认（拒绝登记、从未做过模型工作的条目不参与）。同一判据里存在未核实的检查清理记录（`cleanup_error`）时按检查不可用拒绝。`orbit status` 必须显示「完成复核未通过」的原因，使 `paused` 记录中残留的 `stop_reason`（例如 Root 写的「交付完成」）不被读成已验收。
