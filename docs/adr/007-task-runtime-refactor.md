...
用户决定在已全局启用的所有 Orbit 任务中使用 TypeSafe Jev，按项目允许关闭；宿主环境中的 `TYPESAFE_API_KEY` 即为全局启用条件。它估计过程卡住／偏题或产物值得检查的可能性，再由现有独立检查者核实；不要求 Jev 解释工程根因。这样同时针对过早检查的成本和执行中遗漏、跑偏的质量风险。开发侧完成测试和真实流程验证后才考虑发布，不要求用户承担观察试点。

Jev 的判断不取代固定产物上的独立检查及实际停止证据。程序保留约定时间和 Root 交付时的完整检查，过程疑点交给同一检查角色作聚焦核查；无新证据不反复唤起。TypeSafe 不可用时恢复原调度。外发仅限有界任务内容、近期观察与差异摘要，key 不进入任务记录。具体接口和状态以任务运行合同为准。
