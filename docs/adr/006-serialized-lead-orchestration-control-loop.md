...
- 状态：Accepted；2026-08-17 部分修订——串行边界收窄为 per Task、跨 Task 并行交给 Git branch/worktree，见文末修订记录
- 日期：2026-08-03
- Owner 批准：2026-08-03
...
> **已取代（2026-08-17）**：本段中 control identity 的「project-scoped」定性已收窄为 Task-scoped；stable、immutable、executor 可替换而不改 identity 的语义保留，见文末修订记录。
...
- > **已取代（2026-08-17）**：本条中「初始 task ownership/queue」绑定已按 task-centric 模型收窄——control 不持有 Task 集合，genesis exact bind 单一 Task，见文末修订记录。
...
- > **已取代（2026-08-17）**：本条中「project-wide runtime-subject unique active binding」与跨 lineage registry claim 语义已收窄为 Task 内；同一原子 create 中的 genesis/session/checkpoint 原子性保留，见文末修订记录。
...
> **已取代（2026-08-17）**：本段中「同一 project 的全部 open control lineages」跨 lineage runtime subject binding 校验已按 task-centric 模型收窄为 Task 内校验；每个 control 内单 active LeadSession 保留，见文末修订记录。
...
> **已取代（2026-08-17）**：本段中 executor/control-lineage transfer 与跨 control subject binding 语义已按 task-centric 模型取代——Lead/agent 更换走 Task 内 session replacement 或 handoff；Task 内 terminal→successor 原子性与 fail-closed 语义保留，见文末修订记录。
...
> **已取代（2026-08-17）**：本段中「不同 `lead_control_id` 只有在 task ownership sets 不相交且 active `lead_runtime_subject_ref` sets 也不相交时才可并行」已按 task-centric 模型取代——不同 Task 各自 branch/worktree 天然并行，同一 Task 内串行，见文末修订记录。
...
> **已取代（2026-08-17）**：本段的多 Task 跨 lineage 协调与 ownership/subject disjointness 校验已按 task-centric 模型取代——协调收窄为 Task 内，不同 Task 间不需要 Orbit 层协调，见文末修订记录。
...
> **已取代（2026-08-17）**：本段整段为 cross-control Task transfer（release/acquire）协议，已按 task-centric 模型移除——不存在 cross-control Task transfer，一个 Task 只允许一条 accepted control lineage，见文末修订记录。
...
> **已取代（2026-08-17）**：本段中「project-scoped registry 只承担 control genesis 唯一性、Task ownership 与 transfer 的最小协调约束」已按 task-centric 模型取代——registry 收窄为 Task 内 genesis 唯一性，不存在 project-wide Task ownership registry，见文末修订记录。
...
> **已取代（2026-08-17）**：本段中「control transfer 后必须得到相同 fingerprint」的跨 control 语义随 transfer 移除；retry/checkpoint/session/AgentInstance replacement 不改变 fingerprint 的语义保留，见文末修订记录。
...
> **已取代（2026-08-17）**：本段中「跨 checkpoint/Task transfer lineage continuity」的 transfer 部分已移除；Task 内跨 checkpoint prior chain 保留，见文末修订记录。
...
> **已取代（2026-08-17）**：本段中「Task 在 control lineage 间 transfer 时沿 release/acquire provenance 连续计数」已移除；跨 checkpoint 连续计数保留，见文末修订记录。
...
> **已取代（2026-08-17）**：本段中 project-scoped registry writer 的跨 lineage Task ownership/transfer、canonical runtime subject active-session binding 与全局 Attempt backstop 职责已按 task-centric 模型收窄为 Task 内写入边界，见文末修订记录。
...
3. **disjoint parallelism/transfer**：不同 `lead_control_id` 只有 task sets 与 active provider-verified runtime subject sets 都 disjoint 时可并行；同一 Task/LogicalLead 双 queue ownership/双 tip selection 或同一 runtime subject 双 control binding 被拒绝。Task transfer 必须 old release/suspend checkpoint 在先、new acquire checkpoint 在后；executor transfer 必须 old session terminal/release 在先、new session successor/bind 在后，并保留 exact provenance；不同 project 独立。
   > **已取代（2026-08-17）**：本条的 disjoint parallelism 与 Task/executor transfer 场景已按 task-centric 模型取代——并行改由不同 Task 各自 branch/worktree 提供；「同一 Task 双 lineage ownership 被拒绝」保留（一 Task 一 accepted lineage），见文末修订记录。
...
5. **single active LeadSession/executor**：同一 `lead_control_id` 双 active session、同一 provider/runtime subject 以相同或不同 AgentInstance ID 同时绑定两个 control IDs、别名 subject 绕过、非当前 session dispatch、无 terminal/release predecessor 的 replacement/transfer 均被拒绝；受控 atomic session successor/transfer 可在不并发 active 的前提下复用 executor。
   > **已取代（2026-08-17）**：本条中「同一 runtime subject 绑定两个 control IDs」跨 control 场景与受控 atomic transfer 已按 task-centric 模型取代；Task 内双 active session 拒绝与 terminal→successor replacement 保留，见文末修订记录。
...
10. **third same fingerprint**：fingerprint 必须由 LeadControl/受控 writer 按 canonical version 从 `fingerprint_identity_basis` 产生，并把 per-occurrence refs/digests 另存为不参与 hash 的 `fingerprint_supporting_provenance`。不同 Attempt、checkpoint、outcome record ID/digest 但相同 stable test/check signal 必须得到同一 fingerprint；stable Finding identity 相同而 GateEvaluation ref 更新时也必须相同；typed code 或 stable Finding/test/check/signal subject identity 不同则必须不同。rename/reorder/message wording/AgentInstance alias 不改变 identity；跨 checkpoint/transfer prior chain 必须连续且只用于计数/authorization scope。缺稳定 signal identity 或 provenance 不能支持 basis 时 freeze。第三次相同 fingerprint 缺 provider-verified `task.retry.override` AuthorizationRecord、exact scope binding、user/control-plane issuer 或尝试 replay 时被拒绝。
   > **已取代（2026-08-17）**：本条中「跨 transfer prior chain」语义已移除；跨 checkpoint prior chain 与其余条款保留，见文末修订记录。
...
- > **已取代（2026-08-17）**：本条中「需要并行时使用 task sets 与 runtime subjects 都 disjoint 的独立 `lead_control_id`，或使用独立 project」已按 task-centric 模型取代——并行使用不同 Task 各自的 Git branch/worktree，见文末修订记录。
...
### ProjectPolicy delegation envelope

Lead 的自主范围由 active `ProjectPolicyRevision` 定义：envelope = policy + closure basis + budget ceiling + authority scope。envelope 内 Lead 自主判断（agent/context 选择、继续/暂停、attempt 顺序、ceiling 内预算调整），写入 accepted LeadCheckpoint，不询问用户；只有硬越界、目标变化、风险接受需要 user/control-plane（`needs_user`）。

### 跨 Agent test budget

- `ProjectPolicyRevision` 定义 default（repo default：每 (TaskRevision, WorkUnit) lineage 累计新增测试 ≤ 10 个、≤ 300 行，可配置）与 Lead ceiling；10/300 是仓库初始客户端/default policy 建议，**不是通用 schema correctness 常量**。
- `test.budget.adjust`：Lead 在 policy ceiling 内、作用层级明确的自主调整——adjustment 是 enclosing checkpoint 内可独立 canonicalize 的 typed payload，独立 digest 为 `budget_adjustment_digest`；payload 预像绑定 active policy ref+digest、`budget_scope_type`（`work_unit_lineage`|`task_lineage`）、**exact predecessor checkpoint ref+digest、该 scope 的 predecessor effective-budget-binding digest、absolute requested effective count/LOC ceilings**、supporting refs，**排除 enclosing checkpoint ID/content digest，且不携带 measurement tuple（当前观测只属于每个 effective_budget_binding，不属于授权事实）**；不需用户。
- `test.budget.override`：超过 ceiling 必须 provider-verified、create-only immutable `AuthorizationRecord`（**预先存在**，consuming checkpoint 只引用 ref+digest）。scope 绑定 `budget_scope_type=work_unit_lineage|task_lineage` + active policy ref+digest + project + TaskRevision +（按 scope）exact WorkUnit + **exact authorizing predecessor checkpoint ref+digest 与该 scope 的 predecessor effective-budget-binding digest + absolute requested effective count/LOC ceilings** + `lead_control_id`；**不携带 measurement tuple（当前观测不属于授权事实）**；issuer 只能是 user/control-plane，自报、opaque、replay 无效。**source mode=consume|inherit**：`consume` 只能在首次消费 checkpoint 使用——绑定预先存在的 AuthorizationRecord ref+digest，其 authorizing predecessor/binding 必须匹配，且该 record 从未被消费，此 checkpoint 成为该 override 的 `origin_consuming_checkpoint`；`inherit` 必须绑定 origin consuming checkpoint ref+digest + 原 AuthorizationRecord ref+digest，沿同 project/policy/TaskRevision/scope/`lead_control_id` 的连续 accepted lineage，effective ceiling 不变且无 superseding source——**继承不是 replay**。第二次 consume、跨 lineage/task/policy/scope inherit、跳过 origin、inherit 改变 ceiling 均 fail closed。**WorkUnit 级 override 不得放宽 task 级总预算；task 级 override 不得暗含全部 WorkUnit override；两层分别校验，跨层/跨 scope 重放拒绝。**
- **累计口径（两层，都受 ceiling/override 控制）**：第一层按 TaskRevision+WorkUnit 全 lineage 累计（`work_unit_lineage`）；第二层为 TaskRevision 级总预算（policy 定义，覆盖该 TaskRevision 全部 WorkUnit lineage 之和，`task_lineage`）。retry、换 agent、换 session、换 control、新对话均不清零（与 fingerprint prior chain 连续计数同构）；**拆 WorkUnit 不能重置任何一层保险丝**，任一层超限且无 override 时 runner 进入 `needs_user`。
- **`effective_budget_bindings`（authority 规则）**：受控 writer 在 checkpoint acceptance 时从权威事实确定性派生 canonical bindings——恰好两项、固定顺序 `work_unit_lineage`、`task_lineage`、各 scope 唯一一项；每项 `source_kind` 的权威性：`policy_default` = active policy（canonical null/no-adjust）；`lead_adjustment` = 当前 checkpoint 内可独立求 digest 的 adjust payload 或前序 accepted checkpoint ref+digest+其 `budget_adjustment_digest`（沿 accepted lineage 精确引用，不 latest-wins）；`user_override` = 预先存在的 `AuthorizationRecord` ref+digest。缺任一 scope、重复、乱序、错误 null、非法 source 组合或混用 source fields 均 fail closed；无 adjustment 时 `budget_adjustment_digest` 明确 absent，不伪造空 digest。两层同时生效由两项 bindings 表达；**old/new 校验只适用于 current adjustment 的 authoring checkpoint**：`adjust.old_effective_budget` 必须等于该 scope 在 authoring checkpoint 时的前一 binding，`new` 在 policy ceiling 内；**inherited adjustment** 必须验证 origin payload 在 origin 当时与其 predecessor 匹配、当前 effective ceiling 等于 origin 的 `new`、accepted lineage 连续且无 superseding source。bindings 的派生输入与 hash 语义见 ADR-004 决策七。
- **measurement 状态（canonical，只属于每个 binding 的当前观测）**：每项 binding 固定含 `measurements.test_count` 与 `measurements.test_code_lines`（固定键序），各自 `status=verified|unverified`。verified 必须 `usage>=0` + exact provider/snapshot ref+digest；unverified 必须 usage/ref/digest 为 canonical null，并绑定 **typed `unverified_assessment`（固定字段顺序）：`lead_disposition`、`lead_reason_code`、`lead_supporting_refs`、`review_status`、`review_gate_evaluation_ref`**——**`lead_disposition` 与 `review_status` exact mapping**：pending → `proceed_pending_independent_review`，accepted → `proceed_after_independent_review`，rejected → `replan_after_independent_rejection`；unknown/mismatch fail closed；`lead_supporting_refs` 为 sorted unique exact ref+digest；`review_status=pending|accepted|rejected`：pending 时 `review_gate_evaluation_ref` 为 canonical null，accepted/rejected 时必须 exact independent `GateEvaluation` ref+digest **且该 GateEvaluation 携带 `budget_assessment_result` 绑定被评 binding 所在的前序 accepted checkpoint（`assessed_checkpoint_ref+digest=C_pending`、`assessed_effective_budget_binding_digest`、scope/control/metric statuses）**（构造顺序：先接受 `C_pending` pending checkpoint → 独立 GateEvaluation 评审 `C_pending` 的 exact binding（`assessed_checkpoint_ref+digest=C_pending`，`budget_review_subject_projection` hash 纳入 canonical subject）→ successor `C_reviewed` 消费该 evaluation，binding `review_status`/`lead_disposition` 与 outcome exact match，**不得让 evaluation 绑定 `C_reviewed` 自身**；**stale 判定用 `budget_review_subject_projection`（排除 review_status/lead_disposition/review_gate_evaluation_ref 三字段外的全部字段）byte-identical——只允许三字段按 outcome 从 pending/null 映射，其他字段变化即 stale 需重新 pending→review；不得要求 `C_reviewed` 完整 binding digest 等于 `C_pending`（二者按设计不同）**；`outcome` 与 `review_status`/`lead_disposition` exact mapping；budget subject/result 纳入 canonical subject manifest；GateRequirement 必须是明确要求 budget assessment 的 selector，不得复用普通 implementation gate）且 evaluator independence 成立；**same-checkpoint circular ref、跨 checkpoint/binding 引用、普通无关 GateEvaluation 引用均 fail closed**。禁止 null 当 0 或宣称 within-budget。数值预算机械 pass/overrun 只对 verified metric 派生；无 adapter 的 default dispatch 允许 unverified binding 且 `review_status=pending`（结果表述为 `unverified_pending_review`，**绝不派生 within/over budget**）；`accepted` 后仍是 unverified，只代表比例性审查通过（`proceed_after_independent_review`）；`rejected` → frozen/replan（`replan_after_independent_rejection`）。**授权不携带 measurement**：adjust/override 只引用 predecessor binding digest 间接冻结观测，writer 对新 checkpoint 的 current measurements 单独按 canonical verified/unverified 规则派生并以新 ceiling 判定——授权不把 measurement 伪装成 verified，下一 reconcile 仍暴露真实状态。**来源继承**：`user_override` source 用 `mode=consume|inherit`（见上）；`lead_adjustment` source 保持 current（当前 checkpoint 内 payload）与 inherited（前序 accepted checkpoint ref+digest+其 `budget_adjustment_digest`）的现有精确区分。缺字段/顺序错、pending 带 ref、accepted/rejected 缺 ref、非独立 evaluator、把 pending 当 within-budget、未 review 就 unverified adjust/closure 均 fail closed。**切片顺序**：Slice 2 先支持 unverified pending/default dispatch 并保留 forward contract；依赖独立 review 的 unverified budget adjust 与 closure 在 Slice 4 GateEvaluation 落地前 fail closed；最终 cutover 时 unverified adjust 必须 `review_status=accepted`，user override 可改变 ceiling 但不升级 verified，closure 前也必须 accepted。
- 计数只来自真实 change surface 与 trusted provider（CI/test-runner 的 provider-verified 计数、repository 权威扫描），不信 agent 自报。

### Automatic continuation envelope

round 是 safety fuse，不是 correctness。Lead 可在 ceiling 内收紧/调整，每次调整入 checkpoint（typed adjust payload）；超限分类互斥：**需要 user authority 的硬越界**（预算超 ceiling 无 override、第三次同 fingerprint 无 retry override）→ `needs_user`；**Lead 可自动 replan 的控制异常**（连续两轮零 Delivery delta、immediate freeze 条件）→ `frozen`；继续均需 replan/split/switch/escalate 或对应 user authority。首轮零 Delivery delta 仅当 thesis/agent-context/scope/verification plan 至少一项有证据的实质变化才可授权 successor（本 ADR 止损条款）。软调整不询问用户。

### Bounded runner

保留 `LeadControl.reconcile(authoritative_facts, trigger) -> LeadDecision` 单步深模块；外层由同一公开命令串行驱动：collect authoritative facts → reconcile → 受控 writer 原子 compare-and-append accepted checkpoint → 执行恰好一个允许动作 → 下一轮。无并行 dispatch、无通用 scheduler。停止状态（四种，**互斥**，每次 reconcile 恰好给出一个当前状态，不写双态、无隐式转换；新增授权事实后下一次 reconcile 再恢复）：`completed`（Gate Engine 派生 closure）；`blocked`（外部依赖/authority 缺失且非用户事实，等待外部事实恢复）；`frozen`（**仅** Lead 可在 delegation envelope 内自动 replan/split/switch 的控制异常：assurance-only、连续两轮零 Delivery、scope/blast/review-surface/goal relation 失控、fingerprint identity 不可证明）；`needs_user`（**需要 user authority 的硬越界与风险接受**：超 policy ceiling 且无 `test.budget.override`、第三次同 fingerprint 且无 `task.retry.override`、goal 变化、risk acceptance、protected 变更、确需用户提供/授权/接受风险）。runner 不新增事实源，checkpoint 只存 selection/assessment/delta/decision 的 exact refs。

### Recovery：trigger / provenance / idempotency / 禁补造

- 恢复沿同一 `lead_control_id` 从权威 facts + 唯一 accepted checkpoint tip 重建 queue/selection/decision（本 ADR 场景 13）；缺 runtime subject assertion、fork、双 active 时 fail closed。
- idempotency：attempt_id 预分配 + 单次 `AttemptCreated`；checkpoint 原子 compare-and-append；create-only 对象同 ID 同内容幂等复用；replay 要么同一对象要么失败，不双写。
- **缺权威正文/证据不得补造，且按互斥三态归位**：recovery 所需任何正文、artifact、resolution、checkpoint 或 evidence 缺失时：外部权威事实/服务可恢复 → `blocked`；可自动 replan 的控制异常 → `frozen`；**只有确需用户提供、授权或接受风险才进入 `needs_user`**。禁止从对话/缓存/候选关系合成缺失正文（ADR-003 候选关系不能推进 state）。
...
- > **已取代（2026-08-17）**：本条中「受控原子 transfer」已移除；terminal/release → successor/bind 保留，见文末修订记录。
- 不绕预算/gate：budget 累计跨 recovery 连续；recovery 后 dispatch 仍须满足 predecessor terminal、依赖 ready、gate 要求。

---

## 修订记录（2026-08-17）：串行边界收窄为 per Task，跨 Task 并行交给 Git branch/worktree

**这是产品范围修订，不是本 ADR 原目标的全部达成。** 缩减后的控制合同不得被描述为原 ADR 完整交付。

### 被取代的表述

以下原文保留在上文，但自 2026-08-17 起不再作为 v2 实现权威：

1. 「稳定 control lineage identity」中 control identity 的 project-scoped 定性与多 Task queue 持有前提；
2. genesis 清单中的初始 task ownership/queue 绑定与 project-wide runtime-subject unique active binding、跨 lineage registry claim；
3. 「同一 project 的全部 open control lineages」runtime subject 跨 lineage binding 校验；
4. executor/control-lineage transfer、跨 control subject binding 与 session transfer 协议；
5. 「不同 `lead_control_id` 以 task ownership sets 与 runtime subject sets disjoint 为并行前提」的并行边界；
6. 「LogicalLead 与多任务协调」中的跨 lineage ownership disjointness 校验、Task transfer release/acquire provenance 与 project-scoped registry 最小协调职责；
7. fingerprint 条款中的跨 control transfer 后同 fingerprint、跨 Task transfer lineage continuity 与 transfer prior chain 计数；
8. 「单一 writer 与 control authority」中 registry writer 的跨 lineage 职责；
9. Normative acceptance 场景 3（disjoint parallelism/transfer）的跨 control 部分、场景 5 中跨 control subject 双绑定与 atomic transfer、场景 10 中 transfer prior chain；
10. 后果中「disjoint 独立 `lead_control_id` 并行」、Amendment 引言中「同 project disjoint 并行边界不变」、Recovery 中「受控原子 transfer」。

> 同族未被逐条标注的其余表述（顶层 invariant 的多 Task queue 前提与跨 lineage subject 互斥、场景 1/4/13/15 中的 project-scoped/跨 lineage 措辞、Non-goals 的同 control 并行参照）同样以本节为准。
...
control identity 的 scope 从 project 收窄为 Task：`lead_control_id` 成为 Task 内 authority/decision stream 的稳定 identity。本 ADR 其余核心内容在 Task 内继续有效：串行 invariant（单 active LeadSession/selected WorkUnit/non-terminal Attempt）、checkpoint-before-dispatch、terminal→successor 控制边界、LeadControl 深模块、四层自检、Delivery/Assurance 区分、止损与 fingerprint 语义、bounded runner 与 recovery。跨 Task 并行不再由 Orbit 的 disjointness 协议提供，由 Git branch/worktree 隔离提供。
