...
状态：**唯一整项改版总 TODO**（2026-09-24 建立，持续更新）。[ADR-008](../adr/008-omp-native-collaboration-base.md) 已采纳；`orbit omp`、原生 `task/hub` 接线、独立 OMP 检查组件、单宿主安装与旧路径清理已进入实现和回归阶段。本机曾执行一次安装切换（历史验收安装 0.6.18，现已不存在）；**冻结 #9 目标路径端到端已通过**（0.6.17 staged `0bc4ec93`），M4 整体闭合：冻结 #1–#9 全部取得判定（#5 为用户批准的组合证据 PASS，例外与限制见阻断 2；#6 机械回路 PASS、矛盾夹具内容不计）。路径原型与早期证据见[路径验证票](omp-native-path-probe.md)和[验证记录](../reference/omp-native-path-probe-20260924.md)。本文件是这次改版唯一的执行队列；完成后已完成项退出待办，并把结论归入相应正文。
...
| M2.3 | `install`／`update`／`uninstall`／`doctor` 单宿主化与安装记录切换（只做安全切换，不建旧参数兼容层） | 历史验收安装是 0.6.18（digest `06dba0f1…`，installed_at `2026-09-24T21:55:54Z`；本机当前无该安装；历史构建见验收记录）。构建 H 的 Ruby 钉住已修复 G 的默认 PATH 阻断，并经 `/` 复核；更新失败保留旧版、有存活租约时拒绝卸载已在隔离前缀取得无模型负样本（exit 1 且保留完整安装，证据 `/tmp/orbit-m4-entry-install-negatives.md`）；**update 成功路径已被证明**（构建 P 即由 `orbit update` exit 0 安装，digest `dcf89b00…`，installed_at `18:06:22Z`），未来新构建 update 只作门禁 | [验收记录](../reference/omp-native-m4-acceptance-20260924.md)；冻结验收 8 的本机部分 |
| M2.4 | 上游 OMP 入口版本门与 SDK 固定 | 历史验收为 OMP/SDK 18.2.8。2026-09-26 用户改定入口规则为 OMP ≥18.2.8 可启动，低于或无法解析则拒绝；检查者 SDK 仍精确固定 18.2.8。更高版本通过版本门不等于新版本真实协作与停止链路已验收 | [历史验证记录](../reference/omp-native-path-probe-20260924.md)；[ADR-008 决定 7](../adr/008-omp-native-collaboration-base.md) |
...
| M3.3 | 版本号与安装记录同步（本次源码 0.7.1 patch；安装为历史验收用 0.6.18） | 源码 0.7.1（2026-09-25 先由 0.6.18 升 minor 至 0.7.0，再由 0.7.0 升 patch 至 0.7.1，随 ADR-009 收尾；0.7.1 尚无自身真实样本）；ADR-009 选模真实验收在临时隔离安装的 0.7.0 release 上取得（见[证据](../reference/model-pool-acceptance-20260925.md)）；历史验收安装 0.6.18 digest `06dba0f1…`（installed_at `2026-09-24T21:55:54Z`）本机当前已不存在——如实记录版本文字与安装状态，不把安装切换写成版本号变更 | `check-version.rb` 与版本文字一致 |
...
| M4.3 | 本机真实安装切换与旧全局扩展核对 | **本机曾切换至 0.6.18（历史验收安装，现本机无安装）**（digest `06dba0f1…`，installed_at `2026-09-24T21:55:54Z`；门禁通过）| [验收记录](../reference/omp-native-m4-acceptance-20260924.md)；冻结验收 1、8 的本机部分 |
...
冻结验收 1–9 仍是原条件，不新增。历史验收安装是 0.6.18（digest `06dba0f1…`，installed_at `2026-09-24T21:55:54Z`；本机当前无该安装；`df4cf490` 假阴性 FAIL、`b4e01ef4` 死锁 FAIL、`ea2d9fe5`/`f8f36109`/`dfec3e93` PARTIAL-A；#9 已于 0.6.17 staged `0bc4ec93` 判 PASS，见验收记录）。已关闭的旧阻断不再占用本段：G 的默认 PATH Ruby 阻断已在 H 修复；P2 的 `paused` 是该样本终态，后续 P3 与 L1 已有正常 `complete`。
