...
| 异常退出时协作证据仍可能有缺口 | 可归属任务的 `task/hub` 事件在观察时另写 `.orbit/tasks/<id>/collaboration.jsonl`，不再只依赖 500 条进程缓冲；缓冲仍供 TaskRuntime 消费。写入失败会在恢复后记 `persistence_gap`，无法归属的事件只计数量不混进别的任务。**残余限制**：进程异常退出前尚未完成的异步写入可能没有可恢复证据；旧任务也没有新日志。主动导出的清单需显示已知缺口与不可取得材料，不能声称绝对完整 | 上游提供可恢复事件流或同步确认边界并完成真实异常退出验证，才可消除此剩余不确定性 |
...
| 自动入口失败时的未知请求体 | 已核对的 OMP `messages` 及 `openai-codex` Responses 请求可把恢复提示送入当前模型请求；不能安全识别的 provider 请求体仍通知并中止当前回合，不冒充 Root 已能按提示恢复 | 取得该 provider 的原生请求体与 onPayload 返回契约并做隔离回归后再加入；不靠猜测字段跳过阻断 |
