...
ADR-007 已替代旧架构，旧运行代码和合同已删除。下面 1–6 项作为历史保留，均随旧路径退役，不再导出阶段 J、cross-control 清理或 v2 文档重写任务。

当前已知限制：普通 embedded TUI 缺少对外控制接口；ephemeral/尚未物化会话缺少分页历史；停止确认限于绑定 Root 和原生登记的后台终端。新供应商和其他会话接入只在实际需求及原生能力明确后增加，不为假设场景提前建框架。

Root 运行故障仅记录，不建设自动替换或恢复体系。用户资料和上下文保留，后续操作以用户新的执行授权为准。
