...
> **已停用（2026-08-17，v1 runtime 删除）**：本文描述的 `orbit` 命令（new-task/state/evidence/rules/whoami/dispatch/notice 等 v1 命令面）已全部删除，本文的协议说明（task contract / loop state / rule packs / transport）对应已删除的 v1 实现，**不得作为现行操作指引**。当前唯一入口是 v2 CLI（`orbit v2 --help`，见 `SKILL.md`）。本文保留为 v1 语义的历史参考；完整 v2 运行时文档重写进行中（ADR-005 修订记录）。
...
客户端 readiness 必须匹配明确的主界面标识，不能把通用提示符当作 ready。若 Codex 等客户端停在更新选择或其他启动提示，`orbit start` 应返回非零的 `started_needs_attention`，保留已创建 pane、binding 和 pending runtime，并在 `next` 中给出检查与处理提示；只有真正进入客户端主界面后才能返回 `started_identity_pending`。对仍处于 pending/stale identity 的现有 binding 再次运行 `orbit start` 时，Orbit 会重新读取同一 pane 的主界面状态：提示未处理时继续以非零 `reuse_needs_attention` 阻塞，处理完成后返回 `reuse_identity_pending`，不会为了恢复流程再创建一个 pane；这只证明客户端界面 ready，仍不等于 verified runtime identity 或 direct-dispatch ready。
...
`orbit evidence submit` 是 review/test verdict 的唯一结构化提交入口。report 必须包含 `kind`、`verdict`、`summary`、`source_message_id`、`findings`、`coverage` 和 `artifacts`；CLI 不会从文件名推断 `kind`，也不会把 `status`、`result`、`decision`、`APPROVED` 或 `CHANGES_REQUESTED` 当作 verdict 别名。review report 还必须包含 `quality_outcome_verdict`，review PASS 必须写 `quality_outcome_verdict: pass`；test PASS 必须包含 `test_level`，且不能是 `not_applicable`。`real_path_required: true` 时，test report 还必须先写 `user_outcomes`：journey id、实际步骤/结果、真实 artifact、设备/浏览器/服务版本、未覆盖路径和项目 test hook；缺失时 pass 自动降级为 partial。项目 hook 配置在 `.orbit/test-hooks.yaml`，通过 `orbit test-hook run --task ... --journey ... --json` 调用，Orbit 不内置平台 runner。`coverage` 和 `artifacts` 是字符串列表；`findings` 可以是字符串列表，也可以是 finding mapping。High/Medium finding 必须用 mapping 写出 `severity`、`summary`、`symptom`、`source`、`consequence` 和 `remedy`。可以从 `assets/templates/review-report.yaml`、`assets/templates/design-review-report.yaml` 或 `assets/templates/test-report.yaml` 复制模板后填写；schema 错误时 CLI 会提示字段路径、期望结构、实际类型和模板路径。`verdict: blocked` 会规范化为 `status: partial` record，并可带 `blocked.reason`、`blocked.next_step`、`blocked.owner` 说明阻塞原因和下一步。`source_message_id` 可以指向 Herdr message、pane transcript、CI job、report file 或其他附件；Herdr 文本本身不是权威 verdict，权威 verdict 是由 CLI 写入 evidence manifest 的结构化 record。不要手写 `.orbit/evidence*.json` 里的 review/test record；直接编辑不会生成可信 identity，不能用于关闭 gate。

`artifact_provenance.required: true` 时，自由文本 `artifacts` 只作人读摘要，不能证明 PASS。对实际输出运行 `orbit artifact inspect --path <relative-output> --task <task> --id <stable-id> --producer-command <command> --json`，把生成的 artifact mapping 作为 `artifact_refs`；implementation PASS 同时记录 `--changed-file`、`--verification`（无代码变化时改用 `--no-change-reason`）和 `--artifact-ref @ref.json`。review/test report 的 `implementation_artifact_refs` 必须引用当前 implementation artifact id。validator 会重新检查文件存在、sha256、git head、生成时间和 task revision；输出被覆盖、删除或 task 改版后，旧 PASS 自动失效。长日志、截图和录屏只保留相对路径、hash 与 lifecycle，不复制进 durable summary。
...
| `implemented_not_independently_accepted` | solo 实现与自测已完成，但 fresh-context review/test 尚未通过；可以交接，不能声称完整完成。 |
...
Orbit protocol 不绑定具体终端工具。manual JSON/file protocol 是当前稳定默认路径。Herdr 是独立项目，也是当前唯一的 automatic-preview adapter，只提供 pane start/inspect。其公开 CLI、socket API、integration 和 plugin 不认证调用进程到 pane 的归属，因此 Orbit 当前不提供 direct dispatch、verified runtime identity 或 notice delivery。tmux、zellij、wezterm、CI、GitHub Actions、普通 shell 或远端 shell 也只能作为手动承载环境或文件/payload 投递路径。
...
用 `orbit tools detect --json` 或 `orbit tools doctor --json` 检查 capability mode。当前 Herdr adapter 的真实结果应为 `automatic-preview`，不得把 pane 可见性解释为可信身份。权威任务投递必须使用 manual payload；任何未来 direct dispatch 都需要新的公开上游信任原语和独立真实 E2E，不能由 fixture 开启。
