---
description: Use when adding or renaming a user-visible symbol, file, API, or outward-facing claim, to check that a reader at HEAD can resolve every reference without session transcript or phase labels.
---
...
只处理会进入正式目录、导出符号、public API 或对外交付说明的名字。注释和内部草稿里的过程残渣，除非会泄漏到对外面，否则不作为本任务必改项。
...
先处理会进入正式目录、导出符号、public API 的名字。不要把一次局部改动扩成全仓库历史措辞清扫。
