...
reviewer 的 gate verdict 必须先写成独立 report，再运行 `orbit evidence submit --file <manifest> --report <report> --json`。可以从 `assets/templates/review-report.yaml` 复制模板后填写。不要直接编辑 `.orbit/evidence*.json` 提交 review；直接写 manifest 会绕过身份解析、schema 校验和并发安全写入，不能用于关闭 review gate。
