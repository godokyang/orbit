...
用户在宿主环境配置 `TYPESAFE_API_KEY` 即全局启用 Jev；每个 Orbit 任务可以把有界的原始要求、有效修改、近期宿主观察和产物差异摘要交给 Jev。项目可以单独关闭该外发。Jev 返回“可能卡住”“可能偏题”“当前产物值得完整检查”以及“当前是否可能有适合独立分工的子任务”的概率；程序依据概率和当前任务状态安排独立检查者或提示 Root，不将概率当作问题证据、完成结论或停止授权。有效修改超出预算时保留最新部分并明确标注被省略的范围，不把缺少更早依据的判断当问题证据。
