...
2026-09-25 决定的收尾与接入语义（已启动任务每个用户回合的持久状态短摘要、完成意图缺有效当前终检时的同步拒绝、显式暂停、实际停止前的二次核对、不自动建任务或派发成员）已写入本正文，依据见 [ADR-007](../docs/adr/007-task-runtime-refactor.md) 同日期一节。诊断与实施复盘见 [Zeen 页面走查任务中的 Orbit 流程](../docs/reference/zeen-page-capture-orbit-process-review-20260925.md)；这些条目已随源码实现，隔离真实验收**进行中**，通过项与保留边界见 [Orbit OMP 接入验收记录](../docs/reference/orbit-omp-access-acceptance-20260925.md)——未验收前不得把相应行为当作已验收。
...
用户已经授权执行、独立检查对任务有实际收益且已接入时，Agent 通过宿主提供的 Orbit 工具主动启动任务；讨论和只读问答不启动，简单局部修改通常直接完成；同一任务不重复启动。接入失败必须如实说明，不能用另建 Root 或仅排队消息假装原会话已受控。用户明确要求必须受控执行时，缺少接入能力不能按普通执行绕过。接入资格不等于自动执行：`orbit omp` 只使会话具备接入资格，Orbit 不在会话启动、用户回合或工具调用时自动创建任务，也不自动派发执行成员；启动任务与派发成员分别由 Root 按本节规则显式决定，未启动任务不产生检查成本。
